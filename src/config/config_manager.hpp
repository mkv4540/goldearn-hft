...
class ConfigValue {
   public:
    ...
    explicit ConfigValue(const std::vector<std::string>& value)
    ...
    std::string as_string() const {
        switch (type_) {
            ...
            case ARRAY:
                return array_value_.empty() ? "" : array_value_[0];
        }
        return "";
    }
    ...
    int64_t as_int() const {
        switch (type_) {
            ...
            case ARRAY:
                return array_value_.empty() ? 0 : std::stoll(array_value_[0]);
        }
        return 0;
    }
    ...
    double as_double() const {
        switch (type_) {
            ...
            case ARRAY:
                return array_value_.empty() ? 0.0 : std::stod(array_value_[0]);
        }
        return 0.0;
    }
    ...
    bool as_bool() const {
        switch (type_) {
            case STRING:
                return str_value_ == "true" || str_value_ == "1" || str_value_ == "yes";
            case INTEGER:
                return int_value_ != 0;
            case DOUBLE:
...
class ConfigManager {
   public:
    ...
    std::string get_string(const std::string& full_key,
