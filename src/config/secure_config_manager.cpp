...
// Note: JSON parsing methods are implemented in config_manager.cpp
// This file contains secure extensions to the ConfigManager class
    ...
    try {
        if (json_val.is_string()) {
            ...
            if (str_val.length() > MAX_CONFIG_VALUE_LENGTH) {
                LOG_WARN("ConfigManager: String value truncated to {} characters",
...
bool ConfigManager::validate_json_depth(const nlohmann::json& json_data,
                                        int current_depth,
                                        int max_depth) {
