...
void ExchangeAuthenticator::set_auth_callback(
    ...
    switch (credentials_.method) {
        ...
        case AuthMethod::CERTIFICATE:
            ...
            if (!std::filesystem::exists(credentials_.certificate_path)) {
                LOG_ERROR("ExchangeAuthenticator: Certificate file not found: {}",
                          credentials_.certificate_path);
                return false;
            }
            if (!std::filesystem::exists(credentials_.private_key_path)) {
                LOG_ERROR("ExchangeAuthenticator: Private key file not found: {}",
                          credentials_.private_key_path);
                return false;
            }
            break;
        default:
...
bool ExchangeAuthenticator::authenticate_with_certificate() {
    ...
    try {
        ...
        LOG_INFO("ExchangeAuthenticator: Certificate configuration validated for {}",
                 exchange_name_);

        // For certificate auth, we don't have a token that expires
        credentials_.token_expiry =
            std::chrono::system_clock::now() + std::chrono::hours(24 * 365);  // 1 year
...
bool ExchangeAuthenticator::make_auth_request(const std::string& url,
    ...
    try {
        ...
        if (!conn.send_data(reinterpret_cast<const uint8_t*>(request_str.c_str()),
                            request_str.length())) {
...
bool ExchangeAuthenticator::parse_auth_response(const std::string& response) {
    ...
    // Look for session_token or access_token (standard JSON field names)
    const std::string session_field = "session_token";
    const std::string access_field = "access_token";

    size_t token_pos = json_body.find("\"" + session_field + "\"");
    std::string active_field = session_field;

    if (token_pos == std::string::npos) {
        token_pos = json_body.find("\"" + access_field + "\"");
        active_field = access_field;
    }
    ...
    LOG_INFO("ExchangeAuthenticator: Extracted session token for {}: {}...",
             exchange_name_,
             credentials_.session_token.substr(0, 8));

    return true;
}

bool ExchangeAuthenticator::parse_url(const std::string& url,
                                      std::string& host,
                                      uint16_t& port,
                                      std::string& path) {
        ...
        while (!stop_refresh_) {
            ...
            if (now >= refresh_time) {
                ...
                if (!refresh_token()) {
                    ...
                    LOG_INFO("ExchangeAuthenticator: Token refresh successful for {}",
                             exchange_name_);
...
std::shared_ptr<ExchangeAuthenticator> MultiExchangeAuthManager::get_authenticator(
...
void MultiExchangeAuthManager::set_global_callback(
    std::function<void(const std::string&, bool, const std::string&)> callback) {
