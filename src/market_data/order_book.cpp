...
void OrderBook::add_order(
    uint64_t order_id, char side, double price, uint64_t quantity, Timestamp timestamp) {
    ...
    update_count_++;
    avg_update_latency_ns_ =
        ((avg_update_latency_ns_ * (update_count_ - 1)) + latency_ns) / update_count_;
}
...
void OrderBook::modify_order(uint64_t order_id, uint64_t new_quantity, Timestamp timestamp) {
    ...
    int64_t quantity_delta =
        static_cast<int64_t>(new_quantity) - static_cast<int64_t>(order_info.quantity);
...
void OrderBook::full_refresh(const std::vector<PriceLevel>& bids,
...
double OrderBook::get_imbalance() const {
    ...
    if (bid_qty == 0 && ask_qty == 0)
        return 0.0;
    if (ask_qty == 0)
        return 1.0;  // Maximum bid imbalance
    if (bid_qty == 0)
        return -1.0;  // Maximum ask imbalance
    ...
    for (size_t i = 0; i < MAX_DEPTH; ++i) {
        if (std::abs(bid_levels_[i].price - price) < tick_size_ / 2.0) {
            ...
            int64_t new_quantity =
                static_cast<int64_t>(bid_levels_[i].total_quantity) + quantity_delta;
            if (new_quantity <= 0) {
                // Remove level
...
void OrderBook::update_ask_levels(double price, int64_t quantity_delta, Timestamp timestamp) {
    ...
    for (size_t i = 0; i < MAX_DEPTH; ++i) {
        if (std::abs(ask_levels_[i].price - price) < tick_size_ / 2.0) {
            // Update existing level
            int64_t new_quantity =
                static_cast<int64_t>(ask_levels_[i].total_quantity) + quantity_delta;
            if (new_quantity <= 0) {
                // Remove level
...
void OrderBook::rebuild_depth() {
    ...
    std::sort(bid_levels_.begin(), bid_levels_.end(), [](const PriceLevel& a, const PriceLevel& b) {
    ...
    std::sort(ask_levels_.begin(), ask_levels_.end(), [](const PriceLevel& a, const PriceLevel& b) {
        if (a.total_quantity == 0)
            return false;
        if (b.total_quantity == 0)
            return true;
    ...
    if (side == 'B' || side == 'b') {
        ...
        for (size_t i = 0; i < OrderBook::MAX_DEPTH && quantity > 0; ++i) {
            if (ask_levels[i].total_quantity > 0) {
                uint64_t level_qty =
                    std::min(static_cast<uint64_t>(quantity), ask_levels[i].total_quantity);
        ...
        for (size_t i = 0; i < OrderBook::MAX_DEPTH && quantity > 0; ++i) {
            if (bid_levels[i].total_quantity > 0) {
                uint64_t level_qty =
                    std::min(static_cast<uint64_t>(quantity), bid_levels[i].total_quantity);
