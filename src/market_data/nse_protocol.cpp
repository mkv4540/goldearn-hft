            ...
            case ParserState::WAITING_HEADER: {
                ...
                if (buffer_pos_ >= MIN_MESSAGE_SIZE) {
                    ...
                    if (expected_message_size_ < MIN_MESSAGE_SIZE ||
                        ...
                        LOG_ERROR("NSEProtocolParser: Invalid message size: {}",
                                  expected_message_size_);
                        parse_errors_++;
                        reset_parser_state();
                        continue;
                    }
...
bool NSEProtocolParser::parse_header(const uint8_t* data, MessageHeader& header) {
    ...
    switch (header.msg_type) {
        ...
        case MessageType::HEARTBEAT:
            break;
        default:
            LOG_WARNING("NSEProtocolParser: Unknown message type: {}",
    ...
    switch (header.exchange) {
        ...
        case Exchange::MCX:
            break;
        default:
            LOG_WARNING("NSEProtocolParser: Unknown exchange: {}",
                        static_cast<uint8_t>(header.exchange));
            return false;
    }
...
bool NSEProtocolParser::validate_message(const MessageHeader& header, const uint8_t* payload) {
    ...
    switch (header.msg_type) {
        case MessageType::TRADE: {
            if (header.msg_length !=
                sizeof(MessageHeader) + sizeof(TradeMessage) - sizeof(MessageHeader)) {
                return false;
            }
        ...
        case MessageType::QUOTE: {
            if (header.msg_length !=
                sizeof(MessageHeader) + sizeof(QuoteMessage) - sizeof(MessageHeader)) {
                return false;
            }
            ...
            // Validate quote data
            if (quote.bid_price < 0.0 || quote.bid_price > MAX_PRICE || quote.ask_price < 0.0 ||
                quote.ask_price > MAX_PRICE) {
...
void NSEProtocolParser::dispatch_message(const MessageHeader& header, const uint8_t* payload) {
    switch (header.msg_type) {
        ...
        default:
            LOG_DEBUG("NSEProtocolParser: Unhandled message type: {}",
                      static_cast<uint8_t>(header.msg_type));
            break;
    }
}
...
bool NSESymbolManager::load_symbol_master(const std::string& filename) {
    ...
    if (!file.is_open()) {
        ...
        SymbolInfo reliance{
            1, "RELIANCE", "INE002A01018", InstrumentType::EQUITY, 0.05, 1, 3000.0, 1500.0};
        SymbolInfo tcs{2, "TCS", "INE467B01029", InstrumentType::EQUITY, 0.05, 1, 4500.0, 2250.0};
        SymbolInfo hdfc{
            3, "HDFCBANK", "INE040A01034", InstrumentType::EQUITY, 0.05, 1, 2000.0, 1000.0};
        SymbolInfo nifty{4, "NIFTY", "NIFTY50", InstrumentType::INDEX, 0.05, 1, 25000.0, 15000.0};
        SymbolInfo banknifty{
            5, "BANKNIFTY", "BANKNIFTY", InstrumentType::INDEX, 0.05, 1, 50000.0, 30000.0};
    ...
    while (std::getline(file, line)) {
        ...
        try {
            ...
            info.type = (tokens[3] == "EQUITY")   ? InstrumentType::EQUITY
                        ...
                        : (tokens[3] == "INDEX")  ? InstrumentType::INDEX
...
const NSESymbolManager::SymbolInfo* NSESymbolManager::get_symbol_info(
...
void NSEFeedHandler::register_order_handler(
    std::function<void(const OrderUpdateMessage&)> handler) {
