...
class OrderManager {
   ...
   private:
    ...
    std::vector<ManagedOrder> split_order_for_routing(const ManagedOrder& order,
    ...
    bool validate_modification_parameters(const ManagedOrder& order,
                                          double new_price,
    ...
    std::atomic<std::chrono::seconds> order_cleanup_interval_{
...
class SmartOrderExecution {
   public:
    ...
    bool execute_iceberg(uint64_t symbol_id,
                         OrderSide side,
                         double price,
                         const IcebergParams& params);
    ...
    bool execute_implementation_shortfall(uint64_t symbol_id,
                                          OrderSide side,
