...
struct RiskViolation {
    ...
    RiskViolation(RiskCheckResult type, ViolationSeverity sev, const std::string& desc)
        : violation_type(type), severity(sev), description(desc), symbol_id(0),
          current_value(0.0), limit_value(0.0) {
...
struct PreTradeContext {
    ...
    PreTradeContext() : order(nullptr), current_position(nullptr),
                       current_market_price(0.0), estimated_fill_price(0.0),
                       portfolio_exposure(0.0), strategy_exposure(0.0) {}
};
...
struct PostTradeContext {
    ...
    PostTradeContext() : execution(nullptr), updated_position(nullptr),
                        portfolio_pnl(0.0), strategy_pnl(0.0), var_impact(0.0) {}
};
...
public:
    ...
    const RiskLimits& get_risk_limits() const { return limits_; }
...
private:
    ...
    double calculate_position_correlation(uint64_t symbol_id, const std::unordered_map<uint64_t, double>& positions) const;
...
public:
    ...
    double calculate_historical_var(const std::unordered_map<uint64_t, double>& positions,
