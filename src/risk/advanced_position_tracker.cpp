...
void AdvancedPositionTracker::update_position(const Fill& fill) {
    ...
    if (it == positions_.end()) {
        ...
        LOG_DEBUG("AdvancedPositionTracker: New position - Symbol: {}, Qty: {}, Price: {}", 
                  fill.symbol, position.quantity, fill.price);
    } else {
...
std::vector<PositionInfo> AdvancedPositionTracker::get_strategy_positions(const std::string& strategy_id) const {
    std::shared_lock<std::shared_mutex> lock(positions_mutex_);
...
StrategyMetrics AdvancedPositionTracker::get_strategy_metrics(const std::string& strategy_id) const {
...
double AdvancedPositionTracker::calculate_portfolio_var(uint32_t days, double confidence) const {
    ...
    double z_score = (confidence == 0.95) ? 1.645 : 
                     (confidence == 0.99) ? 2.326 : 1.645;
    
    double time_factor = std::sqrt(static_cast<double>(days));
    double var = portfolio_std * z_score * time_factor;
    
    LOG_DEBUG("AdvancedPositionTracker: Portfolio VaR ({} days, {}%): {}", 
              days, confidence * 100, var);
    
    return var;
}

double AdvancedPositionTracker::calculate_strategy_var(const std::string& strategy_id, uint32_t days) const {
    std::shared_lock<std::shared_mutex> lock(positions_mutex_);
    
    double strategy_variance = 0.0;
    
    for (const auto& [symbol_id, position] : positions_) {
        if (position.strategy_id != strategy_id || position.quantity == 0.0) continue;
...
void AdvancedPositionTracker::set_position_limits(const std::unordered_map<std::string, double>& limits) {
...
std::vector<std::string> AdvancedPositionTracker::check_limit_violations() const {
    ...
    if (portfolio_metrics.gross_exposure > max_gross_exposure_) {
        violations.push_back("Gross exposure limit exceeded: " + 
                           std::to_string(portfolio_metrics.gross_exposure) + 
    ...
    if (portfolio_metrics.portfolio_var_1d > max_portfolio_var_) {
        violations.push_back("Portfolio VaR limit exceeded: " + 
                           std::to_string(portfolio_metrics.portfolio_var_1d) + 
...
void AdvancedPositionTracker::update_portfolio_metrics() {
    ...
    portfolio_metrics_.net_exposure = portfolio_metrics_.total_long_exposure - 
                                     portfolio_metrics_.total_short_exposure;
    portfolio_metrics_.gross_exposure = portfolio_metrics_.total_long_exposure + 
                                       portfolio_metrics_.total_short_exposure;
...
void AdvancedPositionTracker::calculate_position_risk_metrics(PositionInfo& position) {
    ...
    double position_value = std::abs(position.market_value());
    position.position_var_1d = position_value * position.position_volatility * 1.645; // 95% confidence
}
...
double AdvancedPositionTracker::calculate_concentration_risk() const {
    ...
    if (total_exposure == 0.0) return 0.0;
...
std::vector<std::pair<std::string, double>> AdvancedPositionTracker::scenario_analysis() const {
    ...
    scenarios.emplace_back("Sector Rotation (-10%)", stress_test_portfolio(sector_rotation_scenario));
    scenarios.emplace_back("Volatility Spike (-5%)", stress_test_portfolio(volatility_spike_scenario));
    
    return scenarios;
}

double AdvancedPositionTracker::calculate_position_var(const PositionInfo& position, uint32_t days) const {
    double position_value = std::abs(position.market_value());
