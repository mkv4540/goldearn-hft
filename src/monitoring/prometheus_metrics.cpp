...
std::shared_ptr<Counter> MetricsRegistry::create_counter(const std::string& name, const std::string& help) {
...
std::shared_ptr<Gauge> MetricsRegistry::create_gauge(const std::string& name, const std::string& help) {
    std::lock_guard<std::mutex> lock(mutex_);
...
std::shared_ptr<Histogram> MetricsRegistry::create_histogram(const std::string& name, const std::string& help,
...
std::shared_ptr<Summary> MetricsRegistry::create_summary(const std::string& name, const std::string& help) {
    std::lock_guard<std::mutex> lock(mutex_);
...
HFTMetricsCollector::HFTMetricsCollector() {
    ...
    order_latency_histogram_ = registry.create_histogram(
        "goldearn_order_latency_microseconds",
        "Order processing latency in microseconds",
        {1, 5, 10, 25, 50, 100, 250, 500, 1000, 2500, 5000, 10000}
    );
    
    orders_placed_counter_ = registry.create_counter(
        "goldearn_orders_placed_total",
        "Total number of orders placed"
    );
    
    orders_filled_counter_ = registry.create_counter(
        "goldearn_orders_filled_total",
        "Total number of orders filled"
    );
    
    orders_rejected_counter_ = registry.create_counter(
        "goldearn_orders_rejected_total",
        "Total number of orders rejected"
    );
    
    active_orders_gauge_ = registry.create_gauge(
        "goldearn_active_orders",
        "Number of currently active orders"
    );
    
    // Create market data metrics
    market_data_messages_counter_ = registry.create_counter(
        "goldearn_market_data_messages_total",
        "Total number of market data messages processed"
    );
    
    market_data_errors_counter_ = registry.create_counter(
        "goldearn_market_data_errors_total",
        "Total number of market data parsing errors"
    );
    
    market_data_latency_histogram_ = registry.create_histogram(
        "goldearn_market_data_latency_microseconds",
        "Market data processing latency in microseconds",
        {0.1, 0.5, 1, 2, 5, 10, 25, 50, 100, 250, 500}
    );
    
    market_data_rate_gauge_ = registry.create_gauge(
        "goldearn_market_data_rate_messages_per_second",
        "Current market data message rate"
    );
    
    // Create risk metrics
    risk_check_latency_histogram_ = registry.create_histogram(
        "goldearn_risk_check_latency_microseconds",
        "Risk check latency in microseconds",
        {0.5, 1, 2, 5, 10, 20, 50, 100, 200, 500}
    );
    
    risk_violations_counter_ = registry.create_counter(
        "goldearn_risk_violations_total",
        "Total number of risk violations"
    );
    
    total_position_value_gauge_ = registry.create_gauge(
        "goldearn_total_position_value_inr",
        "Total position value in INR"
    );
    
    unrealized_pnl_gauge_ = registry.create_gauge(
        "goldearn_unrealized_pnl_inr",
        "Unrealized P&L in INR"
    );
    
    realized_pnl_gauge_ = registry.create_gauge(
        "goldearn_realized_pnl_inr",
        "Realized P&L in INR"
    );
    
    // Create system metrics
    cpu_usage_gauge_ = registry.create_gauge(
        "goldearn_cpu_usage_percent",
        "CPU usage percentage"
    );
    
    memory_usage_gauge_ = registry.create_gauge(
        "goldearn_memory_usage_percent",
        "Memory usage percentage"
    );
    
    network_bytes_sent_counter_ = registry.create_counter(
        "goldearn_network_bytes_sent_total",
        "Total network bytes sent"
    );
    
    network_bytes_received_counter_ = registry.create_counter(
        "goldearn_network_bytes_received_total",
...
void HFTMetricsCollector::update_system_metrics() {
    ...
    uint64_t current_message_count = market_data_messages_counter_ ? 
        static_cast<uint64_t>(market_data_messages_counter_->value()) : 0;
    
    auto now = std::chrono::steady_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - last_update_time);
    
    if (elapsed.count() > 0) {
        double rate = static_cast<double>(current_message_count - last_message_count) / elapsed.count();
