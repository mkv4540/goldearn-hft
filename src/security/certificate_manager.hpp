...
enum class RotationStatus { SUCCESS, FAILED, IN_PROGRESS, NOT_NEEDED };
...
class CertificateManager {
   public:
    ...
    bool is_rotation_service_running() const {
        return rotation_running_;
    }
    ...
    // Configuration
    void set_rotation_threshold(std::chrono::hours threshold) {
        rotation_threshold_ = threshold;
    }
    void set_callback(CertificateRotationCallback callback) {
        rotation_callback_ = callback;
    }

    // Certificate monitoring
    std::vector<std::string> get_expiring_certificates(
        std::chrono::hours within = std::chrono::hours(24 * 30));  // 30 days
...
class CertificateRotationProcedure {
   public:
    ...
    virtual RotationResult execute_rotation(const std::string& cert_name,
                                            const Certificate& new_cert) = 0;
...
class NSECertificateRotationProcedure : public CertificateRotationProcedure {
   public:
    RotationResult execute_rotation(const std::string& cert_name,
                                    const Certificate& new_cert) override;
    std::string get_procedure_name() const override {
        return "NSE_Certificate_Rotation";
    }
...
class BSECertificateRotationProcedure : public CertificateRotationProcedure {
   public:
    RotationResult execute_rotation(const std::string& cert_name,
                                    const Certificate& new_cert) override;
    std::string get_procedure_name() const override {
        return "BSE_Certificate_Rotation";
    }
